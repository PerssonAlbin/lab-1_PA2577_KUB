...
def stop_and_remove_containers():
    subprocess.run(['python3', 'stop_and_delete.py'])
...
def build_and_run_containers():
    ...
    subprocess.run(['docker', 'run', '-d', '-p', '3001:80', '--name', 'frontend-container', 'nuxt-frontend'], check=True)

    # Build and run backend
    print("Building backend...")
    subprocess.run(['docker', 'build', '-t', 'express-backend', './backend'], check=True)

    print("Running backend container...")
    subprocess.run(['docker', 'run', '-d', '-p', '3000:80', '--name', 'backend-container', 'express-backend'], check=True)
